...
| 3 | `/sdlc-sync-app-design` | Pulls the design comp for this feature's screen and writes it to `design/`. Reports visual and behavioral changes — **read-only on JSX**. Must be on a feature branch. *(UI-touching features only.)* |
